...
outputs:
  ...
  pseudo_B_narrowPeak:
    ...
    type: File?
  pooled_broadPeak:
    outputSource: poolled_call_peak/broadPeak
    type: File?
  pseudo_A_broadPeak:
    outputSource: pseudo_A_call_peak/broadPeak
    type: File?
  pseudo_B_broadPeak:
    outputSource: pseudo_B_call_peak/broadPeak
    type: File?


steps:
  poolled_call_peak:
    run: peak_caller-with-control.cwl
    in:
      input_bam: bam
    ...
    out: [ narrowPeak, broadPeak ]
  pseudo_A_call_peak:
    run: peak_caller-with-control.cwl
    in:
      input_bam: bam_A
      bam_flagstat: bam_flagstat
      control_bam: control_bam_A
      cutoff-analysis: cutoff-analysis
      macs_callpeaks_g: macs_callpeaks_g
      macs_callpeaks_q: macs_callpeaks_q
      nomodel: nomodel
